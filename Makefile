...
# check in contracts/script/output/${CHAINID}/mfssia_avs_deployment_output.json
...
deploy-all-to-anvil-and-save-state: deploy-eigenlayer-contracts-to-anvil-and-save-state deploy-shared-avs-contracts-to-anvil-and-save-state deploy-mfssia-contracts-to-anvil-and-save-state ## deploy eigenlayer, shared avs contracts, and inc-sq contracts 
...
start-aggregator: ## 
	go run aggregator/cmd/main.go --config config-files/aggregator.yaml \
		--mfssia-deployment ${DEPLOYMENT_FILES_DIR}/mfssia_avs_deployment_output.json \
