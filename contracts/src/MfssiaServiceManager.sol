...
contract MfssiaServiceManager is ServiceManagerBase {
    ...
    IMfssiaTaskManager public immutable mfssiaTaskManager;

    /// @notice when applied to a function, ensures that the function is only callable by the `registryCoordinator`.
    modifier onlyMfssiaTaskManager() {
        require(msg.sender == address(mfssiaTaskManager), "onlyMfssiaTaskManager: not from mfssia task manager");
        _;
    }
    ...
    function freezeOperator(address operatorAddr) external override onlyMfssiaTaskManager {
